#include "hexadecimal.h"
#include <string>
#include <algorithm>
#include <cmath>
#include <vector>

namespace hexadecimal 
{
	bool is_hex_number(const char aChar) { return '0' <= aChar && aChar <= '9'; }
	bool is_hex_letter(const char aChar) { return 'a' <= aChar && aChar <= 'f'; }
	bool is_invalid(const char aChar) { return !is_hex_number(aChar) && !is_hex_letter(aChar); }

	int convert_char(const char hexChar)
	{
		if (is_hex_number(hexChar))
			return hexChar - '0';

		if (is_hex_letter(hexChar))
			return (-87) + hexChar;
	}

	int convert(const std::string hexString)
	{
		if (std::any_of(hexString.begin(), hexString.end(), [](char aChar) { return is_invalid(aChar); }))
			return 0;

		int result = 0;
		int power = hexString.size() - 1;

		for (const char& hexChar : hexString) 
		{
			int digit = convert_char(hexChar);
			result += (digit * std::pow(16, power));

			power -= 1;
		}

		return result;
	}

}  // namespace hexadecimal